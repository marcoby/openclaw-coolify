...
import type { RoleProfile, DecisionScope, VisibilityScope, LanguageMode } from "../types/context";
...
interface RoleRow {
  ...
  recipes_allowed: string | null;
  recipes_require_approval: string | null;
...
function rowToRole(row: RoleRow): RoleProfile {
  return {
    ...
    language_mode: row.language_mode as LanguageMode,
...
export interface CreateRoleData {
  ...
  recipes_allowed?: string[];
  recipes_require_approval?: string[];
  language_mode: LanguageMode;
  is_custom?: boolean;
}

export function createRole(data: CreateRoleData): RoleProfile {
  const db = getDb();
  ...
  `).run(
    ...
    JSON.stringify(data.decision_scope || []),
    JSON.stringify(data.visibility || []),
    JSON.stringify(data.recipes_allowed || []),
    JSON.stringify(data.recipes_require_approval || []),
    data.language_mode,
    data.is_custom ? 1 : 0,
    now
  );
...
export function getRoleById(id: string): RoleProfile | null {
  const db = getDb();
...
export function getRolesForCompany(companyId: string): RoleProfile[] {
  const db = getDb();
  const rows = db.prepare("SELECT * FROM roles WHERE company_id = ? ORDER BY is_custom ASC, title ASC").all(companyId) as RoleRow[];
...
export function getRoleByTitle(companyId: string, title: string): RoleProfile | null {
  const db = getDb();
  const row = db.prepare("SELECT * FROM roles WHERE company_id = ? AND title = ?").get(companyId, title) as RoleRow | undefined;
  return row ? rowToRole(row) : null;
}
...
export function updateRole(id: string, data: UpdateRoleData): RoleProfile {
  const db = getDb();
...
export function hasRolesForCompany(companyId: string): boolean {
  const db = getDb();
  const row = db.prepare("SELECT COUNT(*) as count FROM roles WHERE company_id = ?").get(companyId) as { count: number };
  return row.count > 0;
}
