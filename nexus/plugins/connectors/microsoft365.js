 ...
 */
import { BaseConnector, registerConnector, } from "./base";
// ============================================================================
// Configuration
// ============================================================================
const M365_CONFIG = {
...
export class Microsoft365Connector extends BaseConnector {
    config = M365_CONFIG;
    capabilities = M365_CAPABILITIES;
    clientId;
    clientSecret;
    tenantId;
    constructor() {
    ...
    async getCalendarEvents(startDate, endDate) {
        const response = await this.graphRequest(`/me/calendarview?startDateTime=${startDate}&endDateTime=${endDate}&$orderby=start/dateTime`);
    ...
    // Mail Operations
    // --------------------------------------------------------------------------
    async getRecentMails(count = 10) {
        const response = await this.graphRequest(`/me/messages?$top=${count}&$orderby=receivedDateTime desc`);
    ...
    async getTasks() {
        // Get all task lists
        const listsResponse = await this.graphRequest("/me/todo/lists");
        const tasks = [];
        for (const list of listsResponse.value) {
            const tasksResponse = await this.graphRequest(`/me/todo/lists/${list.id}/tasks`);
...
function mapCalendarEvent(raw) {
    return {
        ...
        attendees: raw.attendees?.map((a) => a.emailAddress?.address) || [],
        location: raw.location?.displayName || "",
    };
}
function mapMailMessage(raw) {
    return {
        id: raw.id,
        subject: raw.subject || "",
...
function mapTodoTask(raw, listName) {
    return {
        id: raw.id,
        title: raw.title || "",
        description: raw.body?.content || "",
