    ...
    async chat(request) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new OpenClawError(`OpenClaw request failed (${response.status}): ${errorText}`, response.status);
            }
            const data = (await response.json());
            // Extract content from OpenAI-style response
            const content = data.choices?.[0]?.message?.content;
            if (typeof content !== "string") {
                throw new OpenClawError(`OpenClaw returned unexpected response format: ${JSON.stringify(data).slice(0, 500)}`, 500);
            }
            return content;
        }
        catch (error) {
            ...
            throw new OpenClawError(`OpenClaw request failed: ${error instanceof Error ? error.message : "Unknown error"}`, 500);
        }
    }
    /**
